...
def new_month(command_args: list[str]) -> None:
    ...
    while not user_is_happy:
            ...
            user_is_happy = Prompt.ask("\nAre you happy with the allocation?", choices=["yes", "no"], default="no" if unallocated_existed else "yes") == "yes"
...
def simplify_value_errors(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            print(f"[red bold]Error:[/red bold] It seems you have entered an invalid value. Please try again.")
    return wrapper

@simplify_value_errors
def converse_with_user() -> dict[str, int]:
    print("New month! Let's allocate blocks for the upcoming month.")
    print("Press [cyan bold]enter[/cyan bold] to accept the [cyan bold](default value)[/cyan bold].")
    today = date.today()
    if today.day > 2:
        print(f"Today is {today}. It's probably time to allocate blocks for the next month.")
    ...
    if int(working_days) > days_in_month or int(working_days) <= 0:
        print(f"[red bold]Error:[/red bold] Invalid number of working days. Please try again.")
    ...
    for i, area in enumerate(areas):
        ...
        blocks_for_area = int(Prompt.ask(f"\tHow many blocks for {area}?", default=str(equal_split_from_remaining)))
        if blocks_for_area < 0 or blocks_for_area > remaining_blocks:
            raise ValueError("Invalid value")
        blocks_per_area[area] = blocks_for_area
        print("\n")

    blocks_per_area.update({"Unallocated": total_blocks - sum(blocks_per_area.values())})
...
def display_blocks_table(blocks: dict[str, int], highlight: str | None) -> None:
    ...
    table.add_row(*[str(blocks[area]) if area != highlight else f"[bold]{blocks[area]}[/bold]" for area in blocks.keys()])
